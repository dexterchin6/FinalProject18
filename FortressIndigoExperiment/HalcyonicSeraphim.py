...
def battle(board,boardWidth,boardHeight,teamPlayer):
    ...
    def boardPrint():
        ...
        def printInformation():
            ...
            outputMessage(f"Health: {currentObject.health}",black,int(infoSheet.rect.x + ((infoSheet.image.get_size()[0])/2)*0.15),int(displayHeight*0.42),0)
            outputMessage(f"ATK: {currentObject.attack}",black,int(infoSheet.rect.x + ((infoSheet.image.get_size()[0])/2)*0.15),int(displayHeight*0.45),0)
            outputMessage(f"SPD: {currentObject.speed}",black,int(infoSheet.rect.x + ((infoSheet.image.get_size()[0])/2)*0.15),int(displayHeight*0.48),0)
            outputMessage(f"DEF: {currentObject.defense}",black,int(infoSheet.rect.x + ((infoSheet.image.get_size()[0])/2)*0.15),int(displayHeight*0.51),0)
            outputMessage(f"RES: {currentObject.resistance}",black,int(infoSheet.rect.x + ((infoSheet.image.get_size()[0])/2)*0.15),int(displayHeight*0.54),0)
            outputMessage(f"CRIT: {currentObject.critical}%",black,int(infoSheet.rect.x + ((infoSheet.image.get_size()[0])/2)*0.15),int(displayHeight*0.57),0)
    ...
    def skirmish(attacker,defender,ph,pa,ps,pd,pr,pc,eh,ea,es,ed,er,ec):
        ...
        def initial(pa,pc,eh,ea,es,ed,er,ec,i):
            ...
            def damageCalculate(damage,health,mCritical):
                ...
                if health > 0:
                    outputMessage(f"{defender.nameBattle} has {health} health left.",black,displayWidth/2,displayHeight*(h + 0.5),1)
                else:
                    ifc = 1
                    outputMessage(f"{defender.nameBattle} has no more health left. {defender.speechDeath}",black,displayWidth/2,displayHeight*(h + 0.5),1)
                    deathEnemy = defender
                return health,ifc    
            if i == 0:
                outputMessage(f"{attacker.nameBattle} is attacking {defender.nameBattle} with {attacker.nameWeapon}! {attacker.speechAttack}",black,displayWidth/2,displayHeight*0.3,1)
            elif i == 1:
            ...
            dam -= block
            dam = int(dam)
            eh,ifc = damageCalculate(dam,eh,mCritical)
            pygame.display.flip()
            pygame.time.delay(2000)
        ...
        def counter(ph,pa,ps,pd,pr,pc,ea,ec,i):
            ...
            def damageCalculateRev(damage,health,mCritical):
                ...
                if health > 0:
                    outputMessage(f"{attacker.nameBattle} has {health} health left.",black,displayWidth/2,displayHeight*(h + 0.5),1)
                else:
                    ifc = 1
                    outputMessage(f"{attacker.nameBattle} has no more health left. {attacker.speechDeath}",black,displayWidth/2,displayHeight*(h + 0.5),1)
    ...
    for character in teamEnemy:
        while True:
            chosenX = random.randint(0,boardWidth - 1)
            chosenY = 0
            if board[chosenX][chosenY] == "Empty":
                break
        board[chosenX][chosenY] = character
...
OrigRobin = Character(pygame.image.load("Images/RobinChibi.png").convert_alpha(),pygame.image.load("Images/RobinSkirmish.png").convert_alpha(),"Robin","Robin","Tier I","Resistance Counter","VIFB-U0168 Oblivion","Based on FEH's 'High Deliverer' Robin","Magic","Infantry",2,16,10,8,9,12,2,"Tome","Blarraven","A standard tome.","Time to tip the scales!","Here's how it's done!")
OrigNinian = Character(pygame.image.load("Images/NinianChibi.png").convert_alpha(),pygame.image.load("Images/NinianSkirmish.png").convert_alpha(),"Ninian","Ninian","Tier I","Dancer","VIFB-U0027 Aurora","Based on FEH's 'Oracle of Destiny' Ninian","Dragon","Infantry",2,14,8,7,15,10,4,"Dragonstone","Water Breath","A reflective stone from the ocean.","May this make a difference!","Behold my power!")
#-----------------------------------------------------------------------------------------------------------------#
